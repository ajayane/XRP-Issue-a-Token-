...
//made few additional changes to print the generated seed and use secp256k1SeedFromEntropy. You may use ed25519SeedFromEntropy insted
...
public class IssueToken {
    ...
    public static void main(String[] args)
        ...
        // use these base58EncodedSeed to access the account at https://test.xrptoolkit.com/connect-wallet
        // Create cold and hot KeyPairs -----------------------
        Entropy coldEntropy = Entropy.newInstance();
        String coldBase58EncodedSeed = AddressBase58.encode(coldEntropy.value, Lists.newArrayList(Version.FAMILY_SEED), UnsignedInteger.valueOf(coldEntropy.value.length))
        KeyPair coldWalletKeyPair = Seed.secp256k1SeedFromEntropy(coldEntropy).deriveKeyPair();
        System.out.println("Cold base 58 seed is: " + coldBase58EncodedSeed);
        System.out.println("Cold Address: " + coldWalletKeyPair.publicKey().deriveAddress());
        System.out.println("Cold public key: " + coldWalletKeyPair.publicKey().base16Value());
        System.out.println("Cold private key: " + coldWalletKeyPair.privateKey().naturalBytes().hexValue());

        Entropy hotEntropy = Entropy.newInstance();
        String hotBase58EncodedSeed = AddressBase58.encode(hotEntropy.value, Lists.newArrayList(Version.FAMILY_SEED), UnsignedInteger.valueOf(hotEntropy.value.length))
        KeyPair hotWalletKeyPair = Seed.secp256k1SeedFromEntropy(hotEntropy).deriveKeyPair();
        System.out.println("Hot base 58 seed is: " + hotBase58EncodedSeed);
        System.out.println("Hot Address: " + hotWalletKeyPair.publicKey().deriveAddress());
        System.out.println("Hot public key: " + hotWalletKeyPair.publicKey().base16Value());
        System.out.println("Hot private key: " + hotWalletKeyPair.privateKey().naturalBytes().hexValue());
        ...
        // Create trust line -------------------------------------------------------
        String currencyCode = "FOO"; //set the currency code in ISO format or use ascii to hex converted
